...
<?php if (Cui_is_mobile() ): ?><?php if( dopt('Mobiled_adpost_01_b') ) echo '<div class="banner-post">'.dopt('Mobiled_adpost_01').'</div>'; ?><?php endif ;?>
		<article class="article-content">
			<?php the_content(); ?>
            <?php if(get_post_meta($post->ID, "demo_value", true)){?><div style="width:86%; background:#666; color:#fff; padding:5%; margin:2%;text-indent:0px;">下载地址：<?php echo get_post_meta($post->ID, "demo_value", true);?><br/>提取密码：<?php echo get_post_meta($post->ID, "down_value", true);?></div><?php }?>
...
<div class="article-social">
	...
	<nav class="article-nav">
		...
		<?php if (Cui_is_mobile() ): ?><?php if( dopt('Mobiled_adpost_02_b') )echo '<div id="comment-ad" class="banner-related">'.dopt('Mobiled_adpost_02').'</div>'; ?><?php endif ;?>
