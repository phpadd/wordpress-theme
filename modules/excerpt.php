...
<?php if(is_home()&& dopt('hot_list_check') ){ ?>
		...
		<?php } ?>
<?php if (Cui_is_mobile() ): ?><?php if( dopt('Mobiled_adindex_02_b') ) printf('<div class="banner-sticky">'.dopt('Mobiled_adindex_02').'</div>'); ?><?php endif ;?>
<?php  
...
<?php if( $_thumbnail ){ ?>
	<div class="focus"><a target="_blank" href="<?php the_permalink(); ?>"><img class="thumb" src="<?php echo get_bloginfo("template_url") ?>/timthumb.php?src=<?php echo post_thumbnail_src(); ?>&h=150&w=220&q=90&zc=1&ct=1" alt="<?php the_title(); ?>" /></a></div>
	<?php } ?>
...
<?php if( !is_author() && !$_author ){ ?>
	...
	<?php } ?>
	<?php if( !$_time ){ ?><span class="muted"><i class="fa fa-clock-o"></i> <?php echo get_the_time('Y-m-d') ?></span><?php } ?>
...
<a href="javascript:;" data-action="ding" data-id="<?php the_ID(); ?>" id="Addlike" class="action<?php if(isset($_COOKIE['bigfa_ding_'.$post->ID])) echo ' actived';?>"><i class="fa fa-heart-o"></i><span class="count"><?php if( get_post_meta($post->ID,'bigfa_ding',true) ){ echo get_post_meta($post->ID,'bigfa_ding',true); } else {echo '0';}?></span>喜欢</a></span><?php } ?></p>
	<span class="note"> <?php echo deel_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 90, '...'); ?></span>
</article>
